...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.ServerHost == "" {
		svc.ServerHost = enviro("INCIDENT_LOG_API_MONGODB_HOST", "localhost")
	}

	if svc.ServerPort == 0 {
		port := enviro("INCIDENT_LOG_API_MONGODB_PORT", "27017")
	...
	if svc.UserName == "" {
		svc.UserName = enviro("INCIDENT_LOG_API_MONGODB_USERNAME", "")
	}

	if svc.Password == "" {
		svc.Password = enviro("INCIDENT_LOG_API_MONGODB_PASSWORD", "")
	}

	if svc.DbName == "" {
		svc.DbName = enviro("INCIDENT_LOG_API_MONGODB_DATABASE", "incident-log")
	}

	if svc.Collection == "" {
		svc.Collection = enviro("INCIDENT_LOG_API_MONGODB_COLLECTION", "incident-log")
	}

	if svc.Timeout == 0 {
		seconds := enviro("INCIDENT_LOG_API_MONGODB_TIMEOUT_SECONDS", "10")
...
func (m *mongoSvc[DocType]) connect(ctx context.Context) (*mongo.Client, error) {
	...
	client, err := mongo.Connect(
		ctx,
		options.Client().ApplyURI(uri).SetConnectTimeout(10*time.Second),
	)
	if err != nil {
		return nil, err
	}
...
func (m *mongoSvc[DocType]) UpdateDocument(ctx context.Context, id string, document *DocType) error {
	...
	_, err = collection.ReplaceOne(ctx, bson.M{"id": id}, document)
	return err
}
